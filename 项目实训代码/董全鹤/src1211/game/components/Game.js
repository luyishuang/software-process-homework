...
class Game extends Component{
  ...
  _initImg(){
    ...
    this.setState({
      ...
      this.setState({
        thumb: require('../images/stage1/'+'1.jpg'),
      })
      let tmpImgArr = []
      iniArr.sort(() => {
      ...
      iniArr.forEach((i) => {
        tmpImgArr.push({
          'image': require('../images/stage1/'+'1-'+i+'.jpg'),
          'index': i
        })
      })
      this.setState({
  ...
  handleClickImg(index, i){
    if (this.state.selImg.length === 0) {
      ...
      tmpImgArr[i] = {
        'image': require('../images/stage1/'+'1-'+this.state.selImg[0]+'.jpg'),
        'index': this.state.selImg[0]
      }
      tmpImgArr[this.state.selImg[1]] = {
        'image': require('../images/stage1/'+'1-'+index+'.jpg'),
        'index': index
      }
      this.setState({
        imgArr: tmpImgArr,
  ...
  render(){
    return (
      <div className="bg"  style={{backgroundColor:'#fdfbf4'}}>
        ...
        <GameArea
          ...
          {bg}
          <div  className="ziti">
            <p>曲水流觞，是中国古代民间的一种传统习俗，后来发展成为文人墨客诗酒唱酬的一种雅事。夏历的三月上巳日人们举行祓禊（fuxi)仪式之后，大家坐在河渠两旁，在上流放置酒杯，酒杯顺流而下，停在谁的面前，谁就取杯饮酒，意为除去灾祸不吉。</p>
          </div>
      </div>
    )
  }
